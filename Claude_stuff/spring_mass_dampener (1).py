...
# ---------------------------------------------------------------------------
# Frank-Wolfe parallelism — three independent mechanisms:
#
#  (A) Parallel oracle   – candidate set split across threads; each shard
#      returns its local argmin, the master takes the global winner.
#      Overhead is only worthwhile when m (vertices) >> sample_size, so it
#      is gated behind oracle_n_jobs and oracle_shard_size.
#
#  (B) Multi-start       – k independent FW chains launched from k different
#      seed vertices in parallel; the chain with the smallest residual wins.
#      Helps escape stalls near the boundary. Controlled by n_starts.
#
#  (C) Batch parallelism – batch_point_in_hull dispatches independent queries
#      to a process pool; each worker runs a full single-point FW solve.
#      Uses loky backend (fork-safe, no pickling overhead for numpy arrays).
# ---------------------------------------------------------------------------


def _oracle_shard(V_shard: np.ndarray, r: np.ndarray):
    """
    Linear oracle over one shard of the vertex matrix.
    Returns (local_best_index_in_shard, score).
    """
    scores = V_shard @ r
    local = int(np.argmin(scores))
    return local, float(scores[local])


def _fw_single_start(
    V: np.ndarray,
    x: np.ndarray,
    start_idx: int,
    eps: float,
    ...
    stall_iter: int,
    oracle_n_jobs: int,
    oracle_shard_size: int,
    seed,
):
    """
    One complete FW run from vertex ``start_idx``.
    Optionally parallelises the linear oracle across shards.
    Returns (residual, active_indices, weights, y).
    """
    ...
    # decide once whether to use parallel oracle
    use_parallel_oracle = (
        oracle_n_jobs != 1
        and m > oracle_shard_size * 2          # only worth it for large V
    )

    for _ in range(1, max_iter + 1):

        # ── linear oracle (parallel or serial) ──────────────────────────────
        if sample_size < m:
            cand = rng.integers(0, m, size=sample_size)
        else:
            cand = np.arange(m)

        if use_parallel_oracle and len(cand) > oracle_shard_size:
            # split candidate indices into shards, each shard solved in a thread
            shards = [
                V[cand[i : i + oracle_shard_size]]
                for i in range(0, len(cand), oracle_shard_size)
            ]
            shard_offsets = list(range(0, len(cand), oracle_shard_size))

            results = Parallel(n_jobs=oracle_n_jobs, backend="threading")(
                delayed(_oracle_shard)(shard, r) for shard in shards
            )
            # reconstruct global index
            best_local, best_score = min(results, key=lambda t: t[1])
            # find which shard won and map back to cand
            winning_shard = min(range(len(results)), key=lambda i: results[i][1])
            j = int(cand[shard_offsets[winning_shard] + results[winning_shard][0]])
        else:
...
def fw_membership(
    ...
    starts_n_jobs: int       = 1,
    oracle_n_jobs: int       = 1,
    oracle_shard_size: int   = 256,
):
    """
    Minimise ||y - x||_2 over y in conv(V) via Frank-Wolfe.

    Three independent parallelism levers
    -------------------------------------
    n_starts : int (default 1)
        Number of independent FW chains launched from different seed vertices.
        The chain with the lowest residual is returned.  Values of 4–8 help
        when the hull is high-dimensional and FW tends to stall.
        Set ``starts_n_jobs`` to run them in parallel.

    starts_n_jobs : int (default 1)
        Number of parallel workers for multi-start.  -1 = all CPUs.
        Uses the ``loky`` backend (process-based, safe for numpy).
        Rule of thumb: set equal to n_starts when n_starts > 1.

    oracle_n_jobs : int (default 1)
        Number of threads for the parallel linear oracle.  Only activates
        when m (vertex count) > 2 * oracle_shard_size.  Uses the
        ``threading`` backend because the GIL is released for numpy ops.
        Rule of thumb: useful when m > 5000 and sample_size == m (full scan).

    oracle_shard_size : int (default 256)
        Number of vertices per oracle shard when oracle_n_jobs != 1.
    ...
    # Always include the closest vertex as start 0 (best single-start choice).
    # Additional starts are spread across the vertex set by equidistant stride
    # so they cover different regions of the hull.
    rng      = np.random.default_rng(seed)
    i_closest = int(np.argmin(np.sum((V - x) ** 2, axis=1)))

    if n_starts <= 1:
        start_indices = [i_closest]
    else:
        stride  = max(1, m // (n_starts - 1))
        extras  = [i for i in range(0, m, stride) if i != i_closest][: n_starts - 1]
        start_indices = [i_closest] + extras

    # per-start seeds so runs are reproducible but independent
    child_seeds = rng.integers(0, 2**31, size=len(start_indices))

    # ── run starts ────────────────────────────────────────────────────────────
    run_kwargs = dict(
        V=V, x=x, eps=eps, max_iter=max_iter, sample_size=sample_size,
        prune_tol=prune_tol, stall_iter=stall_iter,
        oracle_n_jobs=oracle_n_jobs, oracle_shard_size=oracle_shard_size,
    )

    if len(start_indices) == 1 or starts_n_jobs == 1:
        # serial path — avoids joblib overhead for the common single-start case
        run_results = [
            _fw_single_start(**run_kwargs, start_idx=si, seed=int(cs))
            for si, cs in zip(start_indices, child_seeds)
        ]
    else:
        run_results = Parallel(n_jobs=starts_n_jobs, backend="loky")(
            delayed(_fw_single_start)(**run_kwargs, start_idx=si, seed=int(cs))
            for si, cs in zip(start_indices, child_seeds)
        )

    # ── pick best result ──────────────────────────────────────────────────────
    best = min(run_results, key=lambda t: t[0])   # minimum residual
    res, idx, w, y = best

    if verbose:
        print(f"[FW] best_res={res:.3e}  starts={len(start_indices)}  "
              f"active={len(idx)}")

    inside = res <= eps
    return inside, {"residual": res, "active_indices": idx, "weights": w, "y": y}
...
def point_in_hull_fw(
    ...
    verbose: bool            = False,
    seed                     = None,
    # ── parallelism (forwarded to fw_membership) ─────────────────────────
    n_starts: int            = 1,
    starts_n_jobs: int       = 1,
    oracle_n_jobs: int       = 1,
    oracle_shard_size: int   = 256,
    ...
    """
    Scalable convex-hull membership test (Frank-Wolfe + optional QP cert).

    Parallelism parameters
    ----------------------
    See ``fw_membership`` docstring for full descriptions of
    n_starts / starts_n_jobs / oracle_n_jobs / oracle_shard_size.

    Quick-start recommendations
    ---------------------------
    * Small hull (m < 500), single query  → defaults (all serial)
    * Large hull (m > 5000), full scan    → oracle_n_jobs=4, oracle_shard_size=512
    * High-dimensional, boundary queries  → n_starts=4, starts_n_jobs=4
    * Batch of N queries                  → use batch_point_in_hull with n_jobs=N
    ...
    inside_fw, fw_info = fw_membership(
        ...
        n_starts=n_starts, starts_n_jobs=starts_n_jobs,
        oracle_n_jobs=oracle_n_jobs, oracle_shard_size=oracle_shard_size,
    )
...
def point_in_hull(
    ...
    cvxpy_solver         = cp.SCS,
    seed                 = None,
    # ── FW parallelism (ignored for cvxpy / linprog) ─────────────────────
    n_starts: int           = 1,
    starts_n_jobs: int      = 1,
    oracle_n_jobs: int      = 1,
    oracle_shard_size: int  = 256,
    ...
    FW parallelism parameters (method='fw' only)
    --------------------------------------------
    n_starts / starts_n_jobs  : multi-start FW (see fw_membership docstring)
    oracle_n_jobs / oracle_shard_size : parallel linear oracle
    """
    if method == "fw":
...
def batch_point_in_hull(
    ...
    **kwargs,
    ...
    """
    Hull membership test for a batch of query points.

    Points are independent so this is embarrassingly parallel.
    Each worker runs a complete single-point solve (bbox → FW → optional QP).

    Parameters
    ----------
    V         : (m, d) vertex array  — read-only, shared across workers
    points    : (N, d) query points
    method    : 'fw' | 'cvxpy' | 'linprog'
    tolerance : membership tolerance
    n_jobs    : number of parallel workers.
                -1 = all CPUs, 1 = serial (no joblib overhead).
                Rule of thumb: n_jobs = min(N, os.cpu_count()).
    **kwargs  : forwarded to ``point_in_hull`` (e.g. n_starts, fw_max_iter)
    ...
    if n_jobs == 1 or N == 1:
        # serial path — avoids joblib process-spawn overhead for tiny batches
        return np.array(
    ...
    # loky backend: process-based, fork-safe, numpy arrays passed without copy
    # via memory-mapped buffers when large enough.
    results = Parallel(n_jobs=n_jobs, backend="loky")(
        delayed(point_in_hull)(V, p, method=method, tolerance=tolerance, **kwargs)
        for p in points
    )
...
class LinearUpdateEnv(gym.Env):
    ...
    fw_eps                : FW convergence tolerance
    fw_sample_size        : FW oracle candidate sample size
    fw_max_iter           : FW iteration cap
    fw_n_starts           : number of independent FW chains (multi-start)
    fw_starts_n_jobs      : parallel workers for multi-start (-1 = all CPUs)
    fw_oracle_n_jobs      : parallel workers for oracle sharding (-1 = all)
    fw_oracle_shard_size  : vertices per oracle shard
    ...
    def __init__(
        ...
        # ── FW parallelism ────────────────────────────────────────────────
        fw_n_starts: int           = 1,
        fw_starts_n_jobs: int      = 1,
        fw_oracle_n_jobs: int      = 1,
        fw_oracle_shard_size: int  = 256,
    ):
        super().__init__()
        ...
        # FW parallelism
        self.fw_n_starts          = fw_n_starts
        self.fw_starts_n_jobs     = fw_starts_n_jobs
        self.fw_oracle_n_jobs     = fw_oracle_n_jobs
        self.fw_oracle_shard_size = fw_oracle_shard_size
    ...
    def _in_hull(self, points_hull: np.ndarray, query: np.ndarray) -> bool:
        ...
        return point_in_hull(
            ...
            n_starts=self.fw_n_starts,
            starts_n_jobs=self.fw_starts_n_jobs,
            oracle_n_jobs=self.fw_oracle_n_jobs,
            oracle_shard_size=self.fw_oracle_shard_size,
        )
...
class DQN(nn.Module):
    ...
    Action bounds are stored as registered buffers and applied via clamp in
    ``forward``, so they move automatically with the model to any device.
    Pass ``action_low`` / ``action_high`` as lists or 1-D array-likes of
    length ``output_size``.  Defaults to (-∞, +∞) (no clamping).
    """
    ...
    def __init__(
        ...
        _low  = action_low  if action_low  is not None else [-float("inf")] * output_size
        _high = action_high if action_high is not None else [ float("inf")] * output_size
        self.register_buffer("action_low",  torch.tensor(_low,  dtype=torch.float32))
        self.register_buffer("action_high", torch.tensor(_high, dtype=torch.float32))

    def forward(self, x):
        return torch.clamp(self.model(x), self.action_low, self.action_high)
...
class RNNActor(nn.Module):
    ...
    move automatically with the model to any device.

    Pass ``action_low`` / ``action_high`` as lists or 1-D array-likes of
    length ``action_size``.  Defaults to (-inf, +inf) (no clamping).
    """
    ...
    def __init__(
        ...
        _low  = action_low  if action_low  is not None else [-float("inf")] * action_size
        _high = action_high if action_high is not None else [ float("inf")] * action_size
        self.register_buffer("action_low",  torch.tensor(_low,  dtype=torch.float32))
        self.register_buffer("action_high", torch.tensor(_high, dtype=torch.float32))
    ...
    def init_hidden(self, batch_size: int):
        # Creates the tensor on the same device as the model weights.
        return torch.zeros(
...
class _TransformerActorBase(nn.Module):
    """
    Shared embedding + encoder scaffold for all Transformer actors.

    Action bounds are stored as registered buffers (move automatically with
    the model to any device).  Pass ``action_low`` / ``action_high`` as
    lists or 1-D array-likes of length ``action_size``.
    Defaults to (-inf, +inf) (no clamping).
    """
    ...
    def init_hidden(self, batch_size: int):
        # Creates the initial hidden tensor on the same device as the model weights.
        return torch.zeros(
            self.num_layers, batch_size, self.hidden_size,
            device=self.action_low.device,
        )
...
class TransformerActor(_TransformerActorBase):
    ...
    def forward(self, state, hidden):
        ...
        q = torch.clamp(self.fc(out[:, 0, :]), self.action_low, self.action_high)
